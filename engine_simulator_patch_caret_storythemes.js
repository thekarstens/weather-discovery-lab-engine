...
function safeLink(url){
  ...
  var STORY_THEME_CLASSES = ['setup','spc','metar','metars','radar','warning','warnings','tornado','reports','report','lightning','satellite','model','hrrr','analysis','blue','cyan','orange','red','purple'];
  function applyStoryTheme(item){
    var theme = inferStoryTheme(item);
    if (storyPanel){
      STORY_THEME_CLASSES.forEach(function(t){ storyPanel.classList.remove('story-theme-' + t); });
      storyPanel.classList.add('story-theme-' + theme);
    }
    return theme;
  }

  var storyCameraUsed = { regional:false, focus:false };
  function storyCameraFromItem(item){
    item = item || {};
    var cam = item.camera || (item.map && item.map.camera) || (item.ui && item.ui.camera) || (item.scene && item.scene.camera) || null;
    var preset = String(item.cameraPreset || item.view || (item.ui && (item.ui.cameraPreset || item.ui.view)) || '').toLowerCase();
    if (!cam && preset){
      if (preset.indexOf('regional') !== -1 || preset.indexOf('setup') !== -1) cam = { center:[43.8,-96.7], zoom:6 };
      else if (preset.indexOf('sioux') !== -1 || preset.indexOf('local') !== -1) cam = { center:[43.55,-96.73], zoom:7 };
      else if (preset.indexOf('south') !== -1 || preset.indexOf('impact') !== -1) cam = { center:[43.35,-97.15], zoom:7 };
    }
    if (!cam && storyIndex === 0 && !storyCameraUsed.regional) cam = { center:[43.85,-97.15], zoom:6, __auto:'regional' };
    if (!cam && !storyCameraUsed.focus){
      var theme = inferStoryTheme(item);
      if (theme === 'radar' || theme === 'warning' || theme === 'tornado') cam = { center:[43.45,-97.05], zoom:7, __auto:'focus' };
    }
    return cam;
  }

  function applyStoryCamera(item){
    if (!item || !map || typeof map.setView !== 'function') return;
    if (document.body.classList.contains('track-active') || document.body.classList.contains('draw-active')) return;
    var cam = storyCameraFromItem(item);
    if (!cam) return;
    var center = cam.center || cam.latLng || ((cam.lat != null && cam.lon != null) ? [Number(cam.lat), Number(cam.lon)] : null);
    if (!Array.isArray(center) || center.length < 2) return;
    var lat = Number(center[0]), lon = Number(center[1]);
    if (!isFinite(lat) || !isFinite(lon)) return;
    var z = Number(cam.zoom);
    if (!isFinite(z)) z = map.getZoom ? map.getZoom() : 6;
    try{ map.setView([lat, lon], z, { animate:true }); }catch(e){}
    if (cam.__auto === 'regional') storyCameraUsed.regional = true;
    if (cam.__auto === 'focus') storyCameraUsed.focus = true;
  }
  ...
  function renderStory(i, panTo){
    ...
    var appliedTheme = applyStoryTheme(item);
    try{
      var leadEl = document.getElementById('simSceneLead');
      if (leadEl) {
        var labels = { setup:'Simulator Intro', spc:'SPC Outlook', metars:'Current Conditions', metar:'Current Conditions', radar:'Radar Analysis', warning:'Warnings', warnings:'Warnings', tornado:'Tornado Warning', reports:'Storm Reports', report:'Storm Reports', lightning:'Lightning', satellite:'Satellite', model:'Futurecast', hrrr:'Futurecast', analysis:'Analysis' };
        leadEl.textContent = (labels[appliedTheme] || item.stepKicker || item.label || 'Current Topic') + (item.utc ? ' • ' + item.utc : '');
      }
    }catch(e){}
    ...
    if (panTo) {
      var beforeLatLng = null;
      try{ beforeLatLng = map.getCenter && map.getCenter(); }catch(e){}
      applyStoryCamera(item);
      if (item.lat != null && item.lon != null && !item.camera && !(item.map && item.map.camera) && !(item.ui && item.ui.camera) && !(item.scene && item.scene.camera)) {
        // Fallback to the old behavior only when no camera preset was applied.
        try{
          if (item.zoom != null && isFinite(item.zoom)) map.setView([item.lat, item.lon], item.zoom, { animate:true });
          else map.panTo([item.lat, item.lon], { animate:true });
        }catch(e){}
      }
    }
  }
  ...
  function normalizeStoryRow(r){
    ...
    return {
      ...
      theme: r.theme || r.stepTheme || r.storyTheme || ((r.ui && r.ui.stepTheme) || ''),
      camera: r.camera || ((r.ui && r.ui.camera) || null) || ((r.map && r.map.camera) || null) || ((scene && scene.camera) || null),
      cameraPreset: r.cameraPreset || r.view || ((r.ui && (r.ui.cameraPreset || r.ui.view)) || ''),
      ...
      stepTheme: inferStoryTheme({ product: scene.product || r.product || r.layer || '', layers: layers, title: r.title || r.label || '', theme: r.theme || r.stepTheme || r.storyTheme || ((r.ui && r.ui.stepTheme) || '') }),
      stepLabel: "",
...
window.setWarningsEnabled = setWarningsEnabled;
  ...
  window.setMasterTime = function(value){
    ...
    // v6.1 Timeline Release Caret support:
    // When enabled by the simulator HTML, the engine may not move past the
    // teacher-controlled release/caret time. This keeps radar, warnings,
    // reports, HRRR, METARs, and all probe logic inside the unlocked window.
    try{
      if (window.__WDL_TIMELINE_RELEASE_ENABLED__ === true && isFinite(Number(window.__WDL_RELEASED_MAX_MS))){
        var maxMs = Number(window.__WDL_RELEASED_MAX_MS);
        if (d.getTime() > maxMs) d = new Date(maxMs);
      }
    }catch(e){}
  ...
  if (_scrub){
    ...
    _scrub.oninput = function(){
      ...
      requestAnimationFrame(function(){
        ...
        try{
          if (window.__WDL_TIMELINE_RELEASE_ENABLED__ === true && isFinite(Number(window.__WDL_RELEASED_MAX_PCT))){
            usePct = Math.min(usePct, Math.max(0, Math.min(1, Number(window.__WDL_RELEASED_MAX_PCT))));
            try{ _scrub.value = String(Math.round(usePct * Number(_scrub.max || 1000))); }catch(e){}
          }
        }catch(e){}
