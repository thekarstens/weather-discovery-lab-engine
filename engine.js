...
function debugActiveProduct(){
  try{
    if (typeof getActiveProductKey === "function") return getActiveProductKey();
  }catch(e){}
  if (window.obsRadarEnabled) return "radar";
  if (window.metarVisible) return "metars";
  if (window.hrrrTempEnabled) return "hrrr";
  if (window.spcDay1Enabled) return "spc";
  return "none";
}

function debugValueClass(val){
  if (val === true) return "debug-v debug-ok";
  if (val === false) return "debug-v debug-off";
  return "debug-v";
}
...
function renderDebugPanel(){
  ...
  var zoom = (window.map && typeof window.map.getZoom === "function") ? window.map.getZoom() : "—";
  var rows = [
    ["Product", debugActiveProduct()],
    ["Time (UTC)", debugFormatTime(typeof curZ !== "undefined" ? curZ : null)],
    ["Zoom", zoom],
    ["Radar", !!window.obsRadarEnabled],
    ["Sweep", !!window.radarSweepEnabled],
    ["Radar Frame", (typeof currentRadarFrameIndex !== "undefined") ? currentRadarFrameIndex : "—"],
    ["Manifest Mode", (typeof useManifestFrameScrubber !== "undefined") ? !!useManifestFrameScrubber : "—"],
    ["METARs", !!window.metarVisible],
    ["HRRR", !!window.hrrrTempEnabled],
    ["SPC", !!window.spcDay1Enabled],
    ["States", !!window.statesEnabled],
    ["Counties", !!window.countiesEnabled],
    ["Story Index", (typeof storyIndex !== "undefined") ? storyIndex : "—"],
    ["Lesson ID", (typeof lessonId !== "undefined" && lessonId) ? lessonId : "default"]
  ];

  grid.innerHTML = rows.map(function(r){
    var v = r[1];
    var cls = debugValueClass(v);
    var text = (typeof v === "boolean") ? (v ? "ON" : "OFF") : String(v);
    return '<div class="debug-k">' + r[0] + '</div><div class="' + cls + '">' + text + '</div>';
  }).join("");
}
...
function initDebugPanel(){
  ...
  var refreshBtn = document.getElementById("debugRefreshBtn");
  if (!panel || !toggleBtn || !refreshBtn) return;

  toggleBtn.onclick = function(){
    panel.classList.toggle("debug-collapsed");
    toggleBtn.textContent = panel.classList.contains("debug-collapsed") ? "Open" : "Hide";
    renderDebugPanel();
  };
  ...
  map.on("zoomend moveend", renderDebugPanel);
  renderDebugPanel();
}
