...
window.setReportsFilter = setReportsFilter;
  ...
  function injectLightningStyles(){
    ...
    style.textContent = `
      ...
      .wdl-lightning-wrap{
        position: relative;
        width: var(--icon-w, 44px);
        height: var(--icon-h, 44px);
        pointer-events: none;
      ...
      .wdl-lightning-img{
        ...
        filter:
          hue-rotate(var(--bolt-hue,0deg))
          saturate(var(--bolt-sat,1.15))
          brightness(var(--bolt-bright,1))
          drop-shadow(0 0 2px rgba(255,255,255,.30))
          drop-shadow(0 0 6px rgba(255,210,90,.28))
          drop-shadow(0 1px 4px rgba(255,245,210,.18));
        mix-blend-mode: screen;
      }
      .wdl-lightning-fresh .wdl-lightning-img{
        animation: wdlLightningPulseLive var(--flash-ms,900ms) ease-in-out infinite;
      }
      .wdl-lightning-recent .wdl-lightning-img{
      ...
      @keyframes wdlLightningPulseLive{
        0%{opacity:.20; transform:translate(-2px,-2px) scale(calc(var(--bolt-scale,1) * 0.90));}
        30%{opacity:1; transform:translate(-2px,-2px) scale(calc(var(--bolt-scale,1) * 1.08));}
        55%{opacity:.35; transform:translate(-2px,-2px) scale(calc(var(--bolt-scale,1) * 0.96));}
        100%{opacity:1; transform:translate(-2px,-2px) scale(var(--bolt-scale,1));}
      }
    `;
  ...
  function lightningTrailColor(ageMs){
    var ratio = Math.max(0, Math.min(1, ageMs / Math.max(1, lightningRecentWindowMs)));
    if (ratio <= 0.45){
      var t1 = ratio / 0.45;
      return {
        hue: 0 + (42 * t1),
        sat: 1.18 - (0.10 * t1),
        bright: 1.08 - (0.18 * t1),
        opacity: 0.96 - (0.18 * t1)
      };
    }
    if (ratio <= 0.80){
      var t2 = (ratio - 0.45) / 0.35;
      return {
        hue: 42 + (235 - 42) * t2,
        sat: 1.08 - (0.28 * t2),
        bright: 0.90 - (0.20 * t2),
        opacity: 0.78 - (0.20 * t2)
      };
    }
    var t3 = (ratio - 0.80) / 0.20;
    return {
      hue: 235 + (285 - 235) * t3,
      sat: 0.80 - (0.45 * t3),
      bright: 0.70 - (0.32 * t3),
      opacity: 0.58 - (0.28 * t3)
    };
  }
  ...
  function createLightningIcon(d, opts){
    ...
    var flashMs = Math.max(550, Math.min(1200, Number((lightningManifest && lightningManifest.style && lightningManifest.style.flashMs) || 900)));
    var tint = fresh
      ? { hue: 0, sat: 1.18, bright: 1.05, opacity: 1 }
      : lightningTrailColor(ageMs);

    var scale = fresh
      ? (1.28 + strength * 0.30)
      : (0.96 + strength * 0.18);

    var width = fresh ? 56 : 40;
    var height = fresh ? 56 : 40;
    var cls = 'wdl-lightning-wrap ' + (fresh ? 'wdl-lightning-fresh' : 'wdl-lightning-recent');
    var html = ''
      + '<div class="' + cls + '"'
      + ' style="--icon-w:' + width + 'px;--icon-h:' + height + 'px;--bolt-scale:' + scale.toFixed(2) + ';--flash-ms:' + flashMs + 'ms;">'
      + '<img class="wdl-lightning-img" src="' + LIGHTNING_ICON_DATA_URI + '"'
      + ' style="--bolt-hue:' + tint.hue.toFixed(1) + 'deg;--bolt-sat:' + tint.sat.toFixed(2) + ';--bolt-bright:' + tint.bright.toFixed(2) + ';--bolt-opacity:' + tint.opacity.toFixed(2) + ';"'
      + ' alt="lightning">'
      + '</div>';
    ...
    return L.divIcon({
      ...
      popupAnchor: [0, -Math.round(height * 0.35)]
    });
  }

  function getLightningRecentStyle(d){
    var strength = lightningStrengthFor(d);
    var op = Math.max(0.18, Math.min(0.88, (productOpacity.lightning || 0.9) * (0.40 + strength * 0.14)));
    return { opacity: op };
  }
  ...
  function setLightningOpacity(op){
    ...
    Object.keys(lightningRecentMarkerIndex).forEach(function(k){
      ...
      try{
        if (m && m.getElement) {
          var el = m.getElement();
          if (el) el.style.opacity = String(Math.max(0.18, productOpacity.lightning * 0.82));
        }
      }catch(e){}
    });
  }
  ...
  function upsertLightningRecent(recent){
    ...
    recent.forEach(function(d){
      ...
      if (!marker){
        marker = L.marker([d.lat, d.lon], { icon:createLightningIcon(d, { fresh:false, ageMs:ageMs }), keyboard:false, interactive:true });
        marker.bindPopup(buildLightningPopup(d), { className:'hrrr-popup' });
        lightningRecentMarkerIndex[d.id] = marker;
        lightningRecentLayer.addLayer(marker);
      } else {
        marker.setLatLng([d.lat, d.lon]);
        marker.setIcon(createLightningIcon(d, { fresh:false, ageMs:ageMs }));
      }
      try{
        var el = marker.getElement && marker.getElement();
        if (el) el.style.opacity = String(style.opacity);
      }catch(e){}
    });
